...
export function* research(
	...
	const aiclient: OpenAI | undefined = ctx.getDependency("aiclient");
	yield ctx.assert(aiclient !== undefined, "aiclient must be available");
	...
	while (true) {
		...
		const message = yield* ctx.run(prompt, aiclient!, messages, depth > 0);
...
resonate.setDependency(
	...
	new OpenAI({
		apiKey:
			"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",
	}),
);
